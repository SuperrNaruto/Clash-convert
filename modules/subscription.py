...
def is_clash_config(content):
    ...
    try:
        ...
        # 检查是否包含Clash配置的关键字段
        return (isinstance(config_dict, dict) and 
                ('proxies' in config_dict or 'Proxy' in config_dict or 'proxy-groups' in config_dict or 'proxy-providers' in config_dict))
    except:
        return False
...
def parse_subscription(content):
    ...
    try:
        if is_clash_config(content):
            # 直接解析Clash配置
            config_dict = yaml.safe_load(content)
            return config_dict.get('proxies', [])
